...
# Import simulation module from parent directory
try:
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from modules import simulation as sim_module
except ImportError:
    sim_module = None
...
# ============================================================
# AUTHENTICATION CHECK - Redirect appropriately based on login status
# ============================================================
# Check if user's session has expired before redirecting
if st.session_state.logged_in and st.session_state.username:
    # Update last activity to track user interaction
    update_last_activity(st.session_state.username)
    
    # Check if session has expired
    is_expired, time_left = is_session_expired(st.session_state.username, SESSION_TIMEOUT_MINUTES)
    ...
    if is_expired:
        ...
        st.session_state.username = None
        st.session_state.role = None
        st.error("⏰ **Session Expired** - Your session has been inactive for 15 minutes. Please log in again.")
        st.switch_page("pages/0_Auth.py")
    else:
        # Session still valid - show time remaining in sidebar
        with st.sidebar:
            st.caption(f"⏱️ Session expires in: {time_left}")

# If user is NOT logged in, show login page from pages/0_Auth.py
if not st.session_state.logged_in:
    st.switch_page("pages/0_Auth.py")

# If user IS logged in, redirect to Disease Selection (main workflow)
...
def restore_active_session():
    """
    Verify and restore active sessions from the database.
    If a user claims to be logged in but has no valid session, force them to log in again.
    """
    ...
    # If user claims to be logged in, verify the session is actually valid
    if st.session_state.logged_in and st.session_state.username:
        try:
            ...
            if session_info:
                ...
                # Session claims to be active but isn't in database - force re-login
                st.session_state.logged_in = False
                st.session_state.username = None
                st.session_state.role = None
            ...
            # Error checking session - force re-login
            st.session_state.logged_in = False
            st.session_state.username = None
            st.session_state.role = None
            return
...
if st.session_state.get("hcc_subtype"):
    ...
    if mode == "🔐 Login" or mode is None:
        ...
        **Step 5: Trial History** 📊
        ...
        with example_cols[0]:
            st.markdown("**Infectious Disease**")
            if st.button("💉 mRNA Vaccine", use_container_width=True, help="COVID-19 mRNA Vaccine Example", key="example_mrna_vaccine"):
                # Merge example design with current design to preserve all keys
                example_data = st.session_state.example_designs["COVID-19 mRNA Vaccine"]
                st.session_state.design.update(example_data)
                st.success("✅ Loaded mRNA Vaccine example!")
            st.caption("_COVID-19 vaccination delivery_")
        
        with example_cols[1]:
            st.markdown("**HCC - Hepatocellular Carcinoma**")
            if st.button("🎯 Cancer Therapy", use_container_width=True, help="Cancer Drug Delivery Example", key="example_cancer_therapy"):
                # Merge example design with current design to preserve all keys
                example_data = st.session_state.example_designs["Cancer Drug Delivery"]
                st.session_state.design.update(example_data)
                st.success("✅ Loaded Cancer Therapy example!")
            st.caption("_Sorafenib drug delivery to liver cancer_")
        
        with example_cols[2]:
            st.markdown("**Genetic Disorders**")
            if st.button("🧬 Gene Therapy", use_container_width=True, help="Gene Therapy Vector Example", key="example_gene_therapy"):
                # Merge example design with current design to preserve all keys
                example_data = st.session_state.example_designs["Gene Therapy Vector"]
                st.session_state.design.update(example_data)
                st.success("✅ Loaded Gene Therapy example!")
            st.caption("_mRNA gene therapy for liver cells_")
...
if "design" not in st.session_state:
    st.session_state.design = {
        ...
        # PK Parameters (required for simulation)
        "dose": 10.0,
        "kabs": 0.5,
        "kel": 0.1,
        "k12": 0.3,
...
if mode == "🔐 Login":
    ...
    with col1:
        st.image("https://cdn.pixabay.com/photo/2017/01/31/23/42/animal-2028258_1280.png", 
                width=80)
    st.header("🔐 NanoBio Studio - Login")
...
# ============================================================
    ...
    if sim_module:
        ...
        st.error("❌ Delivery Simulation module not available. Please ensure modules directory is in parent path.")
